...
## Babel이란?
> 바벨(Babel)은 ES6이상의 코드를 ES5이하 버전으로 변환하기 위해 사용, 즉 *`자바스크립트 컴파일러`*
> <br>
> 바벨을  webpack에서 사용하기 위해서 4가지의 모듈이 필요하다
> - @babel/core => 바벨이 실제 동작하는 모듈
> - @babel/preset-env => 바벨의 지원 스펙을 지정(필요한 플러그인들을 프로젝트 지원 환경에 맞춰 동작 결정해줌)
> - babel-loader => webpack지원을 위해 사용
> - @babel/ployfill => 구형 및 일부 브라우저(IE등)에서 지원하지 않는 js문법 들을 지원할 수 있도록 사용
>                      ,즉 최신 Javascript 새로운 문법들을 구형 자바스크립트로 변경 시켜주는 작업을 하는 모듈, ex) Map,Set,new Promise() 
>
> 위 4가지 모듈 설치후 `.babelrc` 파일(옵션을 설정하기 위한 파일)을 추가 
